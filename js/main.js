document.addEventListener('DOMContentLoaded', () => {
  ...
  if(langSwitcher){
    ...
    langSwitcher.addEventListener('change', ()=>window.location.href = langSwitcher.checked ? 'index.html':'index-hr.html');
  }
  ...
  if(contactForm){
    contactForm.addEventListener('submit', e=>{
      ...
      fetch('https://formspree.io/f/xjkrwlzj',{
        ...
        if(res.ok){
          contactForm.reset();
          Swal.fire({title:'Hvala!', text:'Vaša poruka je uspješno poslana.', icon:'success', confirmButtonText:'Zatvori', background:'#1a1a1a', color:'#eaf0f2', confirmButtonColor:'#447486', iconColor:'#2f6479', timer:2500, timerProgressBar:true});
        } else {
          const data = await res.json().catch(()=>({}));
          throw new Error(data.error || 'Slanje poruke nije uspjelo.');
        }
      }).catch(err=>{
        Swal.fire({title:'Greška!', text:err.message||'Došlo je do problema pri slanju.', icon:'error', confirmButtonText:'U redu', background:'#1a1a1a', color:'#eaf0f2', confirmButtonColor:'#a94442', iconColor:'#dc3545'});
      });
    });
  }
